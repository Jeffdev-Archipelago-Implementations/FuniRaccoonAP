...
class NetworkPlayerCon:
	...
	var slot: int
	var players: Array
	var missing_locations: PackedInt32Array
	var checked_locations: PackedInt32Array
...
class NetworkSlot:
	...
	var game: String
	var type: int
	var group_members: PackedInt32Array
...
class RoomInfo:
	...
	static func from_command(command: Dictionary) -> RoomInfo:
		...
		room_info.tags = PackedInt32Array(command["tags"])
		...
		room_info.games = PackedStringArray(command["games"])
